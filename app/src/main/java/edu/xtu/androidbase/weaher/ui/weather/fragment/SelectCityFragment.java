...
public class SelectCityFragment extends BaseFragment implements ICityView {
...
//        getActivity().
    ...
    protected void getNet(LoadView.IOnNetListener iOnNetListener) {
        ...
        if(localFlag == SelectCityRecycleAdapter.CITY){
            ...
            cityPresenter.showCityListView(localFlag,whereCondition);
            toolBar.setToolTitle("城市选择");
        }else{
                ...
                viewHolder.cardView.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        if(localFlag == SelectCityRecycleAdapter.PROVINCE){
                            ...
                            TerminalToolBarActivity.show(mContext,SelectCityFragment.class,bundle);
                        }else if(localFlag == SelectCityRecycleAdapter.CITY){
                            City city = selectCityRecycleAdapter.cities.get(position);
                            SelectCity selectCity = new SelectCity();
                            selectCity.setId(Long.valueOf(city.getCitySort()));
                            selectCity.setCityName(city.getCityName());
                            DbManager.getInstant().mDaoSession.getSelectCityDao().rx().insert(selectCity);
